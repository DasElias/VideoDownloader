    ...
    for url in urls:
        ...
        try:
            result = subprocess.run(
                ['yt-dlp', '--no-mtime', '--embed-metadata', '--embed-thumbnail', '--no-playlist', '--extract-audio', '--audio-format', 'mp3', '--audio-quality', '0', '-o', '/Users/greta/Music/Downloaded\ by\ MediaHuman/%(title)s.%(ext)s', '--ffmpeg-location', '/opt/homebrew/bin/ffmpeg', url],
